...
function [V,t,mNaF,hNaF,mKDR,mCaH,kV,mKM,ic] = traub_fs_v5(T, I0, gLNa, gLK, gL, gNaF, gKDR, gCaH, gKM, gKv3, EK, C, sigma, ic)
  ...
  %%Just pulling up initial conditions?
  ...
  %%Changing the initial conditions to the last output of the vector?
...
% function res = alpha_mKA(V)    % KA activation [Traub 2003].
%   minf = 1.0 ./ (1.0 + exp( (-V - 60)/8.5 ));
%   taum = 0.185 + 0.5/( exp( (V + 35.8)/19.7 ) + exp( (-V - 79.7)/12.7) );
%   res = minf./taum;
% end
% 
% function res = beta_mKA(V)    % KA activation [Traub 2003].
%   minf = 1.0 ./ (1.0 + exp( (-V - 60)/8.5 ));
%   taum = 0.185 + 0.5/( exp( (V + 35.8)/19.7 ) + exp( (-V - 79.7)/12.7) );
%   res = (1.0 - minf)./taum;
% end
% 
% function res = alpha_hKA(V)    % KA inactivation [Traub 2003].
%   hinf = 1.0 ./ (1.0 + exp( (V+78)/6 ));
%   if V < -63
%       tauh = 0.5 / ( exp( (V+46)/5 ) + exp( (-V - 238)/37.5 ) );
%   else
%       tauh = 9.5;
%   end
%   res = hinf./tauh;
% end
% 
% function res = beta_hKA(V)    % KA inactivation [Traub 2003].
%   hinf = 1.0 ./ (1.0 + exp( (V+78)/6 ));
%   if V < -63
%       tauh = 0.5 / ( exp( (V+46)/5 ) + exp( (-V - 238)/37.5 ) );
%   else
...
% function res = alpha_mCaH(V)    % CaH activation [Traub 2003].
%   res = 1.6 / (1 + exp(-0.072*(V-5)));
% end
% 
% function res = beta_mCaH(V)    % CaH activation [Traub 2003].
%   res = 0.02*(V+8.9) / ( exp((V+8.9)/5) - 1 );
% end
...
% function hinf = hinfKIR(V)
%   Vhalf = -80;
%   k = -12;
%   hinf = 1 ./ (1 + exp((Vhalf - V)/k));
% end